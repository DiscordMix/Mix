import Discord, {Message as DiscordMessage} from "discord.js";
import Log from "../core/Log";

namespace Message {
    export interface IMessageEditOptions {
        readonly message: string;
        readonly title?: string;
        readonly color?: string;
        readonly thumbnailUrl?: string;
        readonly imageUrl?: string;
    }

    export interface IEditableMessage {
        readonly msg: DiscordMessage;

        edit(options: IMessageEditOptions): Promise<DiscordMessage>;
    }

    export class EditableMessage implements IEditableMessage {
        public readonly msg: DiscordMessage;

        // TODO: Verify param "message"'s type.
        public constructor(msg: DiscordMessage | DiscordMessage[]) {
            // TODO: Hotfix.
            if (Array.isArray(msg)) {
                Log.warn("Arrays are not supported as the message parameter, choosing the first element");
                msg = msg[0];
            }

            this.msg = msg;
        }

        /**
         * Edit an already sent message.
         */
        public async edit(options: IMessageEditOptions): Promise<DiscordMessage> {
            // TODO: Default values?
            return this.msg.edit("", new Discord.RichEmbed()
                .setColor(options.color || "")
                .setDescription(options.message)
                .setAuthor(options.title, options.thumbnailUrl)
                .setImage(options.imageUrl || ""));
        }
    }
}
